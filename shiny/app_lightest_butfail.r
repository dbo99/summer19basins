...
server <- function(input, output) {
  ...
  output$reg_plot <- renderPlot({
      ...
      t13 <- t13_csv %>% filter(zone %in% input$zone) %>% mutate(param = "t", dsrce = "prsm13")
      p13 <- p13_csv %>% filter(zone %in% input$zone) %>% mutate(param = "p", dsrce = "prsm13")
      t19 <- t19_csv %>% filter(zone %in% input$zone) %>% mutate(param = "t", dsrce = "prsm19")
      p19 <- p19_csv %>% filter(zone %in% input$zone) %>% mutate(param = "p", dsrce = "prsm19")
      
      
      elev <- elev_csv %>% filter(zone %in% input$zone) %>% uncount(count) %>% 
