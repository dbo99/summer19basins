  ...
  shinyUI(fluidPage(
    ...
    sidebarLayout(sidebarPanel(
      ...
      fluidRow(
        ...
        selectizeInput(
        "zone", "zone", choices = unique(t13_csv$zone), 
...
server <- function(input, output) {
  ...
  output$reg_plot <- renderPlot({
      ...
      t13 <-   reactive({t13_csv %>% filter(zone %in% input$zone) %>% mutate(param = "t", dsrce = "prsm13")  })
      p13 <-   reactive({p13_csv %>% filter(zone %in% input$zone) %>% mutate(param = "p", dsrce = "prsm13")  })
      t19 <-   reactive({t19_csv %>% filter(zone %in% input$zone) %>% mutate(param = "t", dsrce = "prsm19")  })
      p19 <-   reactive({p19_csv %>% filter(zone %in% input$zone) %>% mutate(param = "p", dsrce = "prsm19")  })
      
      
      elev <-   reactive({elev_csv %>% filter(zone %in% input$zone) %>% uncount(count) %>% 
                          mutate(unit = "ft", param = "elev", dsrce = "ned2010") })
      
      df <-   reactive({rbind(elev, t13, p13, t19, p19)})
